...
effective_bead_den = iofrac * den_io + (1.0 - iofrac) * den_ps - den_w
...
def wlcfunc(ext, Lp, Lc, T):
    ...
    Fwlc = 1.0 / (4.0 * (1.0 - z_scaled) ** 2) - 0.25
    for i in range(7):
...
def faxenzcorrection_perp(z, r):
    x = r / z
    return 1.0 / (np.power(x, fzpow) @ fzcoef)
...
def faxenzcorrection_par(z, r):
    x = r / z
    return 1.0 / (np.power(x, fppow) @ fpcoef)
...
def wlcext(f):
    return root_scalar(
        lambda x: (fwlc(x) - f), bracket=[0.0, Lc - 1e-8], fprime=dfwlc, fprime2=d2fwlc
    ).root
...
def milsteinSolver(alpha, betaperp, betapar, dt, N, x0):
    ...
    for i in range(N - 1):
        ...
        dWi = dW[i]
        res[i + 1] = (
            xi + alpha(xi) * dt + b * dWi + 0.5 * bperp * db * (dWi * dWi[2] - dt)
        )
...
def simulate_trace(fmag, fg, gamma, dt, N):
    ...
    def f_det(x):
        ...
        f[0] /= ffaxpar(x[2] + rad)
        f[1] /= ffaxpar(x[2] + rad)
        f[2] /= ffaxperp(x[2] + rad)

        return f

    def f_sto_par(x):
        return np.sqrt(2.0 * kT / (gamma * ffaxpar(x + rad)))

    def f_sto_perp(x):
        return np.sqrt(2.0 * kT / (gamma * ffaxperp(x + rad)))
